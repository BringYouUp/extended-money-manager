...
import { Button, Flex, Input, Select, Spinner, Text } from "@ui";
import { getConvertedValue } from "@utils/common";
			...
			<Form.Items>
				...
				<Flex gap={16}>
					<Form.Item
						...
						label={`Amount
              ${fromCategoryToAccount[0] ? `(${fromCategoryToAccount[0].currency})` : null}`}
					...
					<Form.Item
						...
						label={`Amount
            ${fromCategoryToAccount[1] ? `(${fromCategoryToAccount[1]?.currency})` : null}`}
				...
				<Flex column gap={12}>
					<Button type="submit" theme="primary" disabled={isLoading}>
						{isLoading && loadingData.current?.submitting && <Spinner />}
