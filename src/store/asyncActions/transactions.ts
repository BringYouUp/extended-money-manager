...
export const transactionsGetFilteredTransactions = createAsyncThunk<
	Store.Transaction[],
	{ uid: string; filter: Hooks.UseFilterTransactions.FilterModel }
>(
...
export const transactionsAddTransaction = createAsyncThunk<
	Store.Transaction,
	{
		transaction: Omit<Store.Transaction, Store.OmittedDateFields>;
		uid: string;
		withoutModifyingAccount?: boolean;
	}
>(
	"transactions/transactionsAddTransaction",
	(
		{ transaction, uid, withoutModifyingAccount },
...
export const transactionsEditTransaction = createAsyncThunk<
	Partial<Store.Transaction> &
		Pick<Store.Transaction, "id" | "accountId" | "amount" | "toAccountId">,
	{
		transaction: Partial<Store.Transaction> &
			Pick<Store.Transaction, "id" | "accountId" | "amount" | "toAccountId">;
		uid: string;
	}
>(
				...
				.then((snap) => {
					...
					switch (transaction.type) {
						...
						case "withdraw": {
							if (transaction.deleted) {
								accountData.amount +=
									transactionData.toAmount || transactionData.amount;
							} else {
								accountData.amount -=
									(transaction.toAmount || transaction.amount) -
									(transactionData.toAmount || transactionData.amount);
