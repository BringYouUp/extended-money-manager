...
export const minLength =
	(minLength: number): Hooks.UseForm.Validator =>
	(value: string = "") => {
...
export const maxLength =
	(maxLength: number): Hooks.UseForm.Validator =>
	(value: string = "") => {
...
export const accountAmount = (value: string = "") => {
	if (
		+value !== 0 &&
		(!+value || !/^[0-9]\d*?((\.|,)([1-9]\d?|\d[0-9]?))?$/.test(value)) // 1.23 || 1,23 || 1,03
	) {
		return {
...
export const accountCurrency = (value: string) => {
	const acceptedValues: Shared.Currencies.CurrencySymbols[] =
		PLATFORM_CURRENCIES_LIST.map((item) => item.value);
...
export const transactionType = (value: string) => {
	const acceptedValues: Store.TransactionType[] = [
		"income",
		"transfer",
		"withdraw",
	];
