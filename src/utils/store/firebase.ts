...
export const getRef = {
  ...
  accountsEdit: (uid: string, id: string) =>
    doc(db, "users", uid, "accounts", id),
  categoriesEdit: (uid: string, id: string) =>
    doc(db, "users", uid, "categories", id),
  transactions: (uid: string) => collection(db, "users", uid, "transactions"),
  transactionsEdit: (uid: string, id: string) =>
    doc(db, "users", uid, "transactions", id),
  platform: () => collection(db, "platform"),
  platformCurrency: () => doc(db, "platform", "currency"),
};

export const generateTransactionsQuery = (
  params: Hooks.UseFilterTransactions.FilterModel,
): QueryFieldFilterConstraint[] => {
  let res = [];
